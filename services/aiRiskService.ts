...
export class AIRiskService {
  ...
  private async generateSecureRiskAnalysis(
    ...
    const prompt = `
You are a professional risk assessment expert. Analyze the following risk strictly based on the provided sources.
...
Ensure all recommendations are traceable to the provided sources.`;
  ...
  async generateRiskTreatmentPlan(
    ...
    const prompt = `
As a risk management expert, develop treatment options for the following risk:
...
`;
    ...
    try {
      ...
      return this.parseTreatmentPlan(data.text);
