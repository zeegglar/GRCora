...
const ControlMappingModal: React.FC<ControlMappingModalProps> = ({ isOpen, onClose, control, allControls, mappings, onCreate, onDelete }) => {
  ...
  const [mappingToDelete, setMappingToDelete] = useState<ControlMapping | null>(null);
  ...
  const handleDeleteConfirm = () => {
    if (mappingToDelete) {
        onDelete(mappingToDelete.id);
        setMappingToDelete(null);
    }
  }

  const targetToDelete = mappingToDelete ? getTargetControl(mappingToDelete) : null;
                      ...
                      {mappings.map(m => {
                          ...
                          return (
                              <li key={m.id} className="flex justify-between items-center p-2 bg-slate-900/50 rounded-md">
                                  ...
                                  <button onClick={() => setMappingToDelete(m)} className="text-slate-500 hover:text-red-400 p-1 rounded" aria-label={`Delete mapping to ${target.id}`}>
      ...
      <ConfirmationDialog
        ...
        message={
            <>
              ...
              {targetToDelete && (
                  <div className="mt-3 p-2 bg-slate-900/50 rounded-md text-center">
                    <span className="font-mono text-slate-400 text-sm">{`"${control.id}"`}</span>
                    <span className="mx-2 text-slate-500">&harr;</span>
                    <span className="font-mono text-slate-400 text-sm">{`"${targetToDelete.id}"`}</span>
                  </div>
              )}
