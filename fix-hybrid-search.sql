-- Fixed hybrid search function for Supabase
...
BEGIN
  ...
  SELECT
    ...
    (
      ...
      CASE
        ...
        THEN semantic_weight * (1.0 - (nc.embedding <=> query_embedding))
        ELSE 0.0
      END
    )::FLOAT AS similarity
  ...
  WHERE
    ...
    AND (query_embedding IS NULL OR nc.embedding IS NULL OR (1.0 - (nc.embedding <=> query_embedding)) > match_threshold)
    AND nc.control_id != 'TEST-001'
